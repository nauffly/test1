...
async function renameCurrentWorkspace(nextNameRaw){
  const nextName = String(nextNameRaw || "").trim();
  if(!nextName) throw new Error("Workspace name is required.");
  if(!state.workspaceId) throw new Error("No workspace selected.");
  if(!canManageWorkspace()) throw new Error("Only workspace owner can rename workspace.");

  const rpcTries = [
    ["javi_rename_workspace", { workspace_id: state.workspaceId, workspace_name: nextName }],
    ["javi_rename_workspace", { p_workspace_id: state.workspaceId, p_workspace_name: nextName }],
    ["javi_rename_workspace", { wid: state.workspaceId, name: nextName }]
  ];
  for(const [fn,args] of rpcTries){
    try{ await sbRpc(fn, args); state.workspaceName = nextName; persistWorkspaceToLocalStorage(); return nextName; }catch(_){ }
  }
  ...
  if(!Array.isArray(data) || data.length===0){
    throw new Error("Workspace rename failed. Check Supabase RLS for workspaces update.");
  }
  state.workspaceName = String(data[0].name || nextName);
  persistWorkspaceToLocalStorage();
...
function renderAuth(view){
  ...
  card.appendChild(el("div",{class:"muted small", style:"margin-top:10px"},["Display name can be changed later in Workspace tab."]));
  card.appendChild(row);
  card.appendChild(msg);
